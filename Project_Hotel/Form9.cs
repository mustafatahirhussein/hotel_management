...
namespace Project_Hotel
{
    public partial class Form9 : Form
    {
        ...
        private static Form9 instance;
        public static Form9 getinstance()
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Field(s) are empty!","",MessageBoxButtons.OKCancel,MessageBoxIcon.Error);
                return;
            }

            else if (string.IsNullOrEmpty(textBox_pay.Text))
            {
                MessageBox.Show("Payment should not be left blank!");
            }
            ...
            else if (checkBox1_extra.Checked == true)
            {
                ...
                try
                {
                    ...
                    cmd.CommandText = "insert into food_items values('" + Form8.book_id + "','Breakfast " + comboBox1.Text + " Lunch " + comboBox2.Text + "  Dinner " + comboBox3.Text + " Extra: Drinks " + comboBox_drinks.Text + textBox_quan.Text + "','" + sum_now + "','" + textBox_pay.Text + "','On the way!')";
                    cmd.ExecuteNonQuery();
            ...
            else
            {
                ...
                try
                {
                    conn.Open();
                    SqlCommand cmd = conn.CreateCommand();
                    cmd.CommandText = "insert into food_items values('" + Form8.book_id + "','Breakfast " + comboBox1.Text + " Lunch " + comboBox2.Text + "  Dinner " + comboBox3.Text + "','" + sum_now + "','" + textBox_pay.Text + "','On the way!')";
                    cmd.ExecuteNonQuery();
        ...
        private void comboBox_drinks_SelectedIndexChanged(object sender, EventArgs e)
        {
            textbox_amount_tot.Text = null;
            try
            {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            if(timer1.Interval == 15000)
            {
                ...
                try
                {
                    ...
                    cmd.CommandText = "update food_items set food_status = 'Delivered' where book_id = '"+Form8.book_id+"'";
                    cmd.ExecuteNonQuery();
                }
                catch (Exception ex)
                {
        ...
        public void cancel_order()
        {
            try
            {
                ...
                SqlCommand cmd = conn.CreateCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "select food_status from food_items where book_id = '"+Form8.book_id+"'";
                ...
                while (rd.Read())
                {
                    ...
                    else if (rd["food_status"].ToString() == "On the way!")
                    {
                        ...
                        cmd.CommandText = "UPDATE food_items SET food_status = 'Cancelled' where book_id = '"+Form8.book_id+"'";
                        rd.Close();
                        cmd.ExecuteNonQuery();
                        return;
                    }
    ...
    class already_exist : Exception
    {
        ...
        {
            MessageBox.Show("Entered value already exists", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
